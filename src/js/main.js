...
export const DATA_TABLE = () => {
  ...
  // Calcula la altura de la tabla dependiendo de los elementos agarrados dentro
  const calcHeight = () => {
    let h = '100vh';
    let wh = window.innerHeight;
    let hd = $('.header').innerHeight();
    let bc = $('.breadcrumbs').innerHeight();
    let h1 = $('h1');
    let h1h = h1.innerHeight();
    let rh = h1.next('.row').innerHeight() || 0;
    let tfb = $('.tabla-filtro .bottom').innerHeight();
    const calc = () => wh - (bc + h1h + rh + hd + tfb + 150);
    h = calc();
  ...
  // Aplica la altura a la tabla segun la función calcHeight
  ...
  $('.dataTables_scrollHeadInner').width(
    $('.dataTables_scrollBody > table tbody').width()
  );
  ...
  // Sincroniza los 2 scrolls que se generan en la tabla
  $('.dataTables_scrollBody, .dataTables_scrollHead table').on(
    'scroll',
    function (e) {
      $(
        '.dataTables_scrollBody, .dataTables_scrollHead table'
